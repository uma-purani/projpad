...
focus_mode = st.sidebar.checkbox("Focus Mode (hide distractions)", value=False)
...
if not focus_mode:
    ...
    if st.button("Generate a Suggestion"):
        ...
        if option == "Brain Dump":
            response += "- Start with your core excitement. Then narrow the scope. End with a question you'd like to explore."
        elif option == "Start with Keywords":
            response += "- Define each keyword briefly. Link them together into a theme. Expand with real-world examples."
        elif option == "Describe a Scene":
            response += "- Set the scene vividly. Identify the conflict or core action. Translate it into a topic or problem statement."
        elif option == "Problem-Solution Outline":
            response += "- What’s the problem? Why does it matter? What’s a possible angle or solution you want to explore?"
